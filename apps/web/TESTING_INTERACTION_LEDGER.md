...
| Chat question cards | `crm-a-question` block rendering and answer submit | Structured MCQ blocks render as accessible choices, hide raw JSON, and submit selected answers as normal user turns | `lib/question-blocks.test.ts` (new) + `app/components/chat-message.test.tsx` (new) |
