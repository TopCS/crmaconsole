...
describe("Composio status API", () => {
  ...
  it("GET uses Crm-A Integrations branding for fallback load errors", async () => {
    ...
    expect(body.error).toBe("Failed to load Crm-A Integrations status.");
  });
  ...
  it("POST uses Crm-A Integrations branding for fallback update errors", async () => {
    mockedGetComposioMcpHealth.mockRejectedValueOnce("boom");
    const request = new Request("http://localhost/api/composio/status", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    expect(response.status).toBe(500);
    expect(body.error).toBe("Failed to update Crm-A Integrations status.");
  });
