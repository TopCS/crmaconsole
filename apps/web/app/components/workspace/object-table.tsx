	...
	// Row actions
		...
		(_row: EntryRow): RowAction<EntryRow>[] => {
			...
			actions.push({
				...
				onClick: (r) => {
					const eid = String(r.entry_id ?? "");
					if (eid) {setEditingEntryId(eid);}
				},
				icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /></svg>,
			});
			actions.push({
				label: "Delete",
				variant: "destructive",
	...
	<>
		...
		{(showAddModal || editingEntryId) && (
			<AddEntryModal
				...
				onClose={() => { setShowAddModal(false); setEditingEntryId(null); }}
				onSaved={onRefresh}
			/>
		)}
	</>
...
export function AddEntryModal({
	...
	// Edit mode: load the existing entry and prepopulate the form.
	useEffect(() => {
		if (!entryId) {return;}
		let cancelled = false;
		(async () => {
			try {
				...
				setValues(
					Object.fromEntries(
						Object.entries(data.entry).map(([k, v]) => [k, v == null ? "" : String(v)]),
					),
				);
			} catch { /* ignore */ }
			...
			>
				...
				>
					<h2 className="text-lg font-semibold" style={{ color: "var(--color-text)" }}>
						{isEdit ? "Edit" : "Add"} {displayObjectNameSingular(objectName)}
					</h2>
				...
				{/* Form */}
				{loadingEntry ? (
					<div className="flex-1 px-6 py-10 text-sm" style={{ color: "var(--color-text-muted)" }}>
						Loading…
					</div>
				) : (
				<form
				...
				>
					...
					<button
						...
						style={{ background: "var(--color-accent)", color: "white", opacity: saving || loadingEntry ? 0.7 : 1 }}
					>
						{saving ? "Saving..." : "Save"}
					</button>
				</div>
