...
function integrationLogo(id: CrmAIntegrationId): ReactNode {
  switch (id) {
    case "exa":
      return (
        <img
...
function integrationDisplayNameFromId(id: CrmAIntegrationId, fallbackLabel?: string): string {
  switch (id) {
    case "apollo":
      return "Crm-A Enrichments";
    case "exa":
...
function repairDisplayNameFromId(id: ManagedPluginRepairId): string {
  switch (id) {
    ...
    case "exa":
      return "Exa";
    case "crm-a-ai-gateway":
      return "Crm-A AI Gateway";
    case "crm-a-identity":
      return "Crm-A Identity";
    case "posthog":
      return "PostHog";
  }
}

function integrationDisplayName(integration: CrmAIntegrationState): string {
  return integrationDisplayNameFromId(integration.id, integration.label);
}

const INTEGRATION_DESCRIPTIONS: Record<CrmAIntegrationId, string> = {
...
function IntegrationCard({
  ...
  integration: CrmAIntegrationState;
  isSaving: boolean;
  ...
  const statusText = isSaving
    ? "Saving..."
    : integration.locked
      ? "Unavailable until Crm-A Cloud is ready"
      : description;
...
type CrmAIntegrationsSectionProps = {
  data?: IntegrationsState | null;
  loading?: boolean;
  error?: string | null;
  savingId?: CrmAIntegrationId | null;
  repairing?: boolean;
  ...
  excludeIntegrationIds?: readonly CrmAIntegrationId[];
  onToggle?: (integration: CrmAIntegrationState, enabled: boolean) => void;
  onRetry?: () => void;
  onRepair?: () => void;
};

export function CrmAIntegrationsSection(props: CrmAIntegrationsSectionProps = {}) {
  ...
  const [savingId, setSavingId] = useState<CrmAIntegrationId | null>(null);
  ...
  const handleToggle = useCallback(async (integration: CrmAIntegrationState, enabled: boolean) => {
    if (props.onToggle) {
