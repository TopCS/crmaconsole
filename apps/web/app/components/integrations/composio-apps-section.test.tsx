...
const statusPayload: {
  ...
  summary: {
    ...
    verified: true,
    message: "Crm-A Integrations is healthy.",
  },
  config: {
  ...
  it("shows MCP repair bar only when status is unhealthy", async () => {
    const warningStatus = {
      ...
      summary: {
        ...
        verified: false,
        message: "Crm-A Integrations is configured, but a live agent session could not see the tools directly.",
      },
      liveAgent: {
    ...
    await waitFor(() => {
      expect(screen.getByText("Crm-A Integrations is configured, but a live agent session could not see the tools directly.")).toBeInTheDocument();
    });
  ...
  it("does not show a repair bar when live verification is still pending", async () => {
    const pendingStatus = {
      ...
      summary: {
        level: "healthy" as const,
        verified: false,
        message: "Crm-A Integrations is configured and the gateway is reachable. Live-agent verification is pending.",
      },
      liveAgent: {
