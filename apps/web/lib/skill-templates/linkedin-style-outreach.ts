...
export const linkedinStyleOutreach = defineSkillTemplate({
  ...
  interviewQuestions: [
    ...
    {
      id: "send-policy",
      prompt: "Should Crm-A Console draft messages only, or send when rules are met?",
      required: true,
      options: [
  ...
  skillInstructions: [
    "Accept prospects from HubSpot/Crm-A CRM records, enrichment results, uploaded files, pasted names, Gmail relationship history, or manual search criteria; use LinkedIn when connected and useful for profile context.",
    ...
    "For scheduled runs, use idempotency checks against HubSpot/Crm-A CRM notes, Gmail history, prior Crm-A runs, and connected app history so the same prospect is not messaged twice for the same campaign.",
