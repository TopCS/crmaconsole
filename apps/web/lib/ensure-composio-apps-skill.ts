...
const integrationsSkillEntry = MANAGED_SKILLS.find((s) => s.name === "crm-a-integrations");
...
/**
 * Copy `skills/crm-a-integrations` from the shipped package into each configured
...
export function ensureComposioAppsSkillInWorkspaces(): void {
  ...
  const packageRoot = resolveCrmAPackageRoot();
  if (!packageRoot) {
    return;
  }
  const stateDir = resolveOpenClawStateDir();
  const sourceSkillFile = join(packageRoot, "skills", "crm-a-integrations", "SKILL.md");
  const sourceHash = sha256File(sourceSkillFile);
  if (!sourceHash) {
    return;
  }
  for (const workspaceDir of discoverWorkspaceDirs(stateDir)) {
    const skillFile = join(workspaceDir, "skills", "crm-a-integrations", "SKILL.md");
