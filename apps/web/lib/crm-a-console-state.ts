/**
 * Per-workspace `.crm-a-console/` persistence: onboarding state, Composio
...
export type OnboardingCrmACloud = {
  source: "cli" | "web";
  skipped: boolean;
  configuredAt: string;
};
...
function crmAConsoleFilePath(filename: string, workspaceName?: string | null): string {
  return join(resolveCrmAConsoleDir(workspaceName), filename);
}
  ...
  return {
    ...
    identity:
      raw.identity && typeof raw.identity === "object"
        ...
        : undefined,
    crmACloud:
      raw.crmACloud && typeof raw.crmACloud === "object"
        ? (raw.crmACloud as OnboardingCrmACloud)
        : undefined,
    connections:
...
export function readOnboardingState(workspaceName?: string | null): OnboardingState {
  const path = crmAConsoleFilePath(ONBOARDING_FILENAME, workspaceName);
...
export function writeOnboardingState(
  ...
  writeJsonFileAtomic(crmAConsoleFilePath(ONBOARDING_FILENAME, workspaceName), next);
  return next;
}
...
export function readConnections(workspaceName?: string | null): ConnectionsFile {
  return readJsonFile<ConnectionsFile>(
    crmAConsoleFilePath(CONNECTIONS_FILENAME, workspaceName),
    defaultConnections(),
  );
}
...
export function writeConnection(
  ...
  writeJsonFileAtomic(crmAConsoleFilePath(CONNECTIONS_FILENAME, workspaceName), next);
  return next;
}
...
export function readSyncCursors(workspaceName?: string | null): SyncCursors {
  return readJsonFile<SyncCursors>(
    crmAConsoleFilePath(SYNC_CURSORS_FILENAME, workspaceName),
    defaultSyncCursors(),
  );
}
...
export function writeSyncCursors(
  ...
  writeJsonFileAtomic(crmAConsoleFilePath(SYNC_CURSORS_FILENAME, workspaceName), next);
  return next;
}
...
export function readPersonalDomainsOverrides(
  ...
  const raw = readJsonFile<unknown>(
    crmAConsoleFilePath(PERSONAL_DOMAINS_FILENAME, workspaceName),
    null,
  );
...
export function writePersonalDomainsOverrides(
  ...
  writeJsonFileAtomic(crmAConsoleFilePath(PERSONAL_DOMAINS_FILENAME, workspaceName), next);
  return next;
}
...
export function readEmailBodyHydrationAttempted(
  ...
  const raw = readJsonFile<unknown>(
    crmAConsoleFilePath(EMAIL_BODY_HYDRATION_FILENAME, workspaceName),
    null,
  );
  if (!raw || typeof raw !== "object") {
    return new Set();
...
export function markEmailBodyHydrationAttempted(
  ...
  writeJsonFileAtomic(crmAConsoleFilePath(EMAIL_BODY_HYDRATION_FILENAME, workspaceName), next);
  return next;
}
