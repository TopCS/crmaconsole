...
function mockGatewaySearch(responsePayload: Record<string, unknown>) {
  process.env.CRM_A_API_KEY = "crm-a-test-key";
...
describe("buildIdentityPrompt", () => {
  ...
  it("includes Crm-A Integrations skill path and guidance", () => {
    const prompt = buildIdentityPrompt(workspaceDir);
    expect(prompt).toContain(path.join(workspaceDir, "skills", "crm-a-integrations", "SKILL.md"));
    expect(prompt).toContain("Crm-A Integrations");
    expect(prompt).not.toContain("Composio MCP");
    expect(prompt).toContain("Never");
    expect(prompt).toContain("crm_a_search_integrations");
    expect(prompt).toContain("crm_a_execute_integrations");
  });
  ...
  it("teaches the agent to emit direct composio connect links for any app", () => {
    ...
    expect(prompt).toContain("always call `crm_a_search_integrations`");
    ...
    expect(prompt).toContain("crm-a://composio/connect");
  ...
  it("prefers Crm-A Integrations over gog without workspace cache files", () => {
    const prompt = buildIdentityPrompt(workspaceDir);
    expect(prompt).toContain("use the **Crm-A Integrations** tools directly");
  ...
  it("references Crm-A Console branding, not OpenClaw (prevents identity confusion)", () => {
    const prompt = buildIdentityPrompt(workspaceDir);
    expect(prompt).toContain("You are **Crm-A Console**");
    expect(prompt).toContain("always use **Crm-A Console** (not OpenClaw)");
  });
});
...
describe("buildIdentityPrompt composio cache files", () => {
  ...
  const originalCrmAApiKey = process.env.CRM_A_API_KEY;
  const originalCrmAGatewayUrl = process.env.CRM_A_GATEWAY_URL;

  afterEach(() => {
    globalThis.fetch = originalFetch;
    if (originalCrmAApiKey !== undefined) {
      process.env.CRM_A_API_KEY = originalCrmAApiKey;
    } else {
      delete process.env.CRM_A_API_KEY;
    }
    if (originalCrmAGatewayUrl !== undefined) {
      process.env.CRM_A_GATEWAY_URL = originalCrmAGatewayUrl;
    } else {
      delete process.env.CRM_A_GATEWAY_URL;
    }
    if (tmp) {
  ...
  it("does not expose cached tool-index contents in runtime integration guidance", () => {
    tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-composio-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    expect(prompt).toContain("Connected App Tools (Crm-A Integrations)");
    expect(prompt).toContain("live integration schema");
    expect(prompt).toContain("crm_a_search_integrations");
...
describe("register", () => {
  ...
  it("does not register handler when plugin is explicitly disabled (respects config)", () => {
    const api = {
      config: { plugins: { entries: { "crm-a-identity": { config: { enabled: false } } } } },
      on: vi.fn(),
    };
    register(api);
  ...
  it("handler returns system context when workspace is configured", () => {
    ...
    expect(result).toEqual({
      prependSystemContext: expect.stringContaining("Crm-A Console"),
    });
  });
  ...
  it("registers the Crm-A Integrations search tool when the workspace is configured", () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-register-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(path.join(tmp, "skills", "crm-a-integrations"), { recursive: true });
    writeFileSync(
      path.join(tmp, "skills", "crm-a-integrations", "SKILL.md"),
    ...
    expect(api.registerTool).toHaveBeenCalledWith(
      expect.objectContaining({ name: "crm_a_search_integrations" }),
      { name: "crm_a_search_integrations", optional: true },
    );
    expect(getRegisteredTool(api as any, "crm_a_execute_integrations")).toBeUndefined();
  ...
  it("searches GitHub integration tools for recent PR requests", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-github-search-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    mockGatewaySearch({
      items: [
    ...
    expect(payload.instruction).toContain("crm_a_execute_integrations");
  ...
  it("searches across connected apps and ranks the best tool matches", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-search-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    mockGatewaySearch({
      items: [
  ...
  it("returns gateway-backed Crm-A Integrations search results with full schemas", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-gateway-search-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    writeFileSync(
      path.join(tmp, "composio-tool-index.json"),
      JSON.stringify({
        ...
        managed_tools: ["crm_a_search_integrations", "crm_a_execute_integrations"],
        connected_apps: [
          {
            toolkit_slug: "stripe",
            toolkit_name: "Stripe",
  ...
  it("requires account selection when multiple connected accounts match the same app", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-multi-account-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    mockGatewaySearch({
      items: [
  ...
  it("forwards toolkit and limit filters to gateway-backed searches", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-filtered-search-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    process.env.CRM_A_API_KEY = "crm-a-test-key";
    process.env.CRM_A_GATEWAY_URL = "https://gateway.example.com";
  ...
  it("returns static fallback recipes when a connected toolkit search has no gateway matches", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-no-match-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    mockGatewaySearch({
      items: [],
  ...
  it("returns a direct connect link when the requested app is not connected", async () => {
    const tmp = path.join(
      os.tmpdir(),
      `crm-a-identity-connect-required-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    mkdirSync(tmp, { recursive: true });
    mockGatewaySearch({
      items: [],
    ...
    expect(payload.action_link_markdown).toBe(
      "[Connect Slack](crm-a://composio/connect?toolkit=slack&name=Slack)",
    );
