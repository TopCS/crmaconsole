...
function formatComposioToolCheatSheet(index: ComposioToolIndexFile): string {
  const lines: string[] = [
    "## Connected App Tools (Crm-A Integrations)",
    "",
    "Crm-A Integrations is the configured integration layer for these connected apps. Always search first, inspect the returned full schemas and plan guidance, then execute the selected tool via `composio_call_tool`.",
    "",
