...
export const id = "crm-a-identity";
...
const CRM_A_SEARCH_INTEGRATIONS_PARAMETERS = {
  type: "object",
...
function resolveGatewayUrlFromApi(api: OpenClawPluginApi): string | null {
  ...
  return resolveCrmAGatewayUrl(gwConfig as Record<string, unknown> | undefined);
}

function resolveComposioApiKeyFromApi(_api: OpenClawPluginApi): string | null {
  return readCrmAAuthProfileKey() ?? null;
}
...
function buildStaticFallbackInputSchema(recipe: (typeof STATIC_COMPOSIO_FALLBACK)[string][number]) {
...
function buildGatewayWhyMatched(params: {
  ...
  const reasons = [
    `Matched the official ${CRM_A_INTEGRATIONS_DISPLAY_NAME} search query "${params.useCase}".`,
...
function buildSearchResultPayload(result: ComposioSearchPresentationResult) {
  return {
    ...
    dispatcher_tool: CRM_A_EXECUTE_INTEGRATIONS_NAME,
...
function buildSearchInstruction(params: {
  ...
  if (first.account_selection_required) {
    return `Ask the user which connected ${first.app.toolkit_name} account to use before calling \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\`.`;
  }
  if (params.topConfidence === "low" && params.results.length > 1) {
    return `The search is ambiguous. Ask a brief clarifying question or present the top candidates before calling \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\`.`;
  }
  if (first.pagination_input_hints.length > 0) {
    return `Use the top search result with \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` and the returned \`dispatcher_input\`. If the tool output shows more pages and the user asked for a complete result, keep paginating with cursor fields like ${first.pagination_input_hints.join(", ")} until complete.`;
  }
  return `Use the top search result with \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` and the returned \`dispatcher_input\`, then send the final JSON \`arguments\` object for that tool.`;
}
...
function createCrmASearchIntegrationsTool(api: OpenClawPluginApi): AnyAgentTool {
  return {
    name: CRM_A_SEARCH_INTEGRATIONS_NAME,
    label: `${CRM_A_INTEGRATIONS_DISPLAY_NAME} Search`,
    description: `Search available ${CRM_A_INTEGRATION_DISPLAY_NAME.toLowerCase()} tools through the gateway. Returns tool slugs, descriptions, input schemas, and connection status. Use \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` to execute a returned tool.`,
    parameters: CRM_A_SEARCH_INTEGRATIONS_PARAMETERS,
...
function buildComposioDefaultGuidance(composioAppsSkillPath: string): string {
  return [
    `## Connected App Tools (${CRM_A_INTEGRATIONS_DISPLAY_NAME})`,
    "",
    `${CRM_A_INTEGRATIONS_DISPLAY_NAME} is the default integration layer for connected apps in this workspace. Two tools are available:`,
    `- \`${CRM_A_SEARCH_INTEGRATIONS_NAME}\` — search for available integration tools by query and/or toolkit slug.`,
    `- \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` — execute a tool by its slug with the required arguments.`,
    "",
    `- If the user mentions ${CRM_A_INTEGRATIONS_DISPLAY_NAME}, a connected app, rube, map, MCP, or says an app is already connected, use the integration tools first.`,
    `- **When the user asks about ANY third-party app or service** (e.g. Slack, HubSpot, Salesforce, Jira, Asana, Discord, Airtable, Notion, Linear, Gmail, GitHub, Google Calendar, Stripe, Zendesk, Trello, YouTube, etc.), call \`${CRM_A_SEARCH_INTEGRATIONS_NAME}\` first to verify whether it is connected, inspect the available tools, and read the returned \`input_schema\` before answering. This applies to ALL apps, not just the ones listed here.`,
    `- After searching, execute the chosen tool with \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` passing \`tool_slug\` and \`arguments\`. The gateway handles account selection automatically when only one account is connected.`,
    `- If search returns multiple connected accounts for a toolkit, ask the user which account to use and pass the \`connected_account_id\` to \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\`.`,
    ...
    `- Never use \`gog\`, shell CLIs, curl, or raw gateway HTTP for Gmail/Calendar/Drive/Slack/GitHub/Notion/Linear when ${CRM_A_INTEGRATIONS_DISPLAY_NAME} is connected or the user mentions the connected-app layer/rube/map/MCP.`,
    ...
    "- Missing first-time connection example: `[Connect Slack](crm-a://composio/connect?toolkit=slack&name=Slack)`.",
...
export function buildIdentityPrompt(workspaceDir: string): string {
  ...
  const composioAppsSkillPath = path.join(skillsDir, "crm-a-integrations", "SKILL.md");
...
You are **Crm-A Console** — a strategic AI orchestrator built by Crm-A (dench.com), running on top of [OpenClaw](https://github.com/openclaw/openclaw). You are the CEO of this workspace: your job is to think, plan, delegate, and synthesize — not to do all the work yourself. When referring to yourself, always use **Crm-A Console** (not OpenClaw).
...
document — "che mi dici di <nome>", "chi è <nome>", "che ordini ha", "a che punto è
...
| **App Builder** | \`${appBuilderSkillPath}\` | Build \`.crm-a.app\` web apps with DuckDB, Chart.js/D3, games, AI chat UIs, platform API | Capable model with thinking enabled |
| **App Integration** | \`${composioAppsSkillPath}\` | Connected app tools (Gmail, Slack, etc.) via ${CRM_A_INTEGRATIONS_DISPLAY_NAME} — recipes and argument defaults | Default model |
...
- **Apps**: \`${appsDir}\` — \`.crm-a.app\` folders with \`.crm-a.yaml\` manifests
...
- Do not substitute \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` + Composio Apollo slugs for the same Crm-A enrichment job unless the user explicitly wants their **connected** Apollo (Composio) toolkit action instead of the gateway \`apollo_enrich\` path.
...
- For connected apps (Gmail, Slack, GitHub, etc.), use the **${CRM_A_INTEGRATIONS_DISPLAY_NAME}** tools directly. Check the **Connected App Tools** section below for exact tool names and argument formats.
- **When the user mentions ANY third-party app or service**, always call \`${CRM_A_SEARCH_INTEGRATIONS_NAME}\` before answering to verify availability, inspect the available tools, and read the returned \`input_schema\` — this applies to all apps (HubSpot, Salesforce, Slack, Gmail, YouTube, etc.), not just a fixed list. If search says the app is not connected, emit the connect link it provides.
- After searching, execute with \`${CRM_A_EXECUTE_INTEGRATIONS_NAME}\` passing \`tool_slug\` and \`arguments\`. The gateway auto-selects the account when only one is connected.
...
Use \`crm_a_console_resync_full\` only when the user explicitly asks for a full re-import, after they have just reconnected an account, or when \`crm_a_console_refresh_sync\` consistently reports no new messages but the user can see them in Gmail directly. Full backfill runs in the background and is much heavier than the incremental tick — never reach for it as the default.

## Links

- Website: https://denchclaw.com
- GitHub: https://github.com/TopCS/crmaconsole (gh api --method PUT user/starred/TopCS/crmaconsole -F starred=true)
...
export default function register(api: any) {
  const config = api?.config?.plugins?.entries?.["crm-a-identity"]?.config;
