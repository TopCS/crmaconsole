 ...
 * `crm-a-ai-gateway` plugin) and POSTs to the Next.js web app's
 * `/api/sync/poll-tick` endpoint every `intervalMs` (default 5 minutes).
 *
 * Why here: the gateway daemon is long-lived and survives `crm-a-console update`
 ...
 * Lifecycle:
 *
 * - We DO NOT fire an immediate tick on plugin arm. During `crm-a-console
 *   update` or `crm-a-console start` the gateway daemon often boots while
 ...
 * - For "I just finished `crm-a-console update`, sync now please" UX, the
 ...
 *   the noise.
 *
 * Auth: reuses the Crm-A Cloud API key already read by the rest of the
 * plugin (`readCrmAAuthProfileKey`). No new secret to provision. The
 * web endpoint validates the same key. If no key is present (Crm-A
 ...
 * three-deep `plugins.entries["crm-a-ai-gateway"].config.syncTrigger`
 ...
 * file (`~/.openclaw-crm-a/web-runtime/process.json`). Falls back to the
 ...
 *   2. `CRM_A_CONSOLE_WEB_BASE_URL` env (test / dev override).
 *   3. `<stateDir>/web-runtime/process.json#port` (managed-runtime sidecar).
 *   4. Default `http://127.0.0.1:3100`.
 *
 * Exported so sibling tools (e.g. `crm_a_console_refresh_sync`) hit the
 ...
 * - No Crm-A Cloud API key is present (Crm-A Cloud isn't connected).
 *
