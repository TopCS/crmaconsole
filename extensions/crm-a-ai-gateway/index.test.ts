 ...
 * Mirrors the shape `crm-a-ai-gateway/index.ts`'s `register()` reads.
 */
...
describe("crm-a-ai-gateway composio bridge", () => {
  ...
  it("strips the raw composio MCP server and registers the Crm-A Integrations execute bridge", async () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-state-"));
    ...
    // tools (e.g. crm_a_console_refresh_sync, crm_a_console_resync_full) are
    // added in the same `register()` pass — we only care here that the
    // composio bridge tool is among them.
    expect(tools.map((tool) => tool.name)).toEqual(
      expect.arrayContaining(["crm_a_execute_integrations"]),
    );
    expect(api.config.mcp).toBeUndefined();
    expect(info).toHaveBeenCalledWith(
      "[crm-a-ai-gateway] registered crm_a_execute_integrations bridge tool",
    );

    const bridgeTool = tools.find(
      (tool: { name: string }) => tool.name === "crm_a_execute_integrations",
    );
  ...
  it("registers a stable generic schema for crm_a_execute_integrations", () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-state-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    writeAuthProfiles(stateDir, "dc-key");
  ...
  it("requires tool_slug and skips gateway execution when it is missing", async () => {
    ...
    expect(result.content[0]?.text).toContain("crm_a_search_integrations");
  });
});
...
describe("crm-a-ai-gateway sync-trigger", () => {
  const originalFetch = globalThis.fetch;
  const originalStateDir = process.env.OPENCLAW_STATE_DIR;
  const originalWebBaseUrlEnv = process.env.CRM_A_CONSOLE_WEB_BASE_URL;
  let stateDir: string | undefined;
  ...
  afterEach(() => {
    ...
    if (originalWebBaseUrlEnv !== undefined) {
      process.env.CRM_A_CONSOLE_WEB_BASE_URL = originalWebBaseUrlEnv;
    } else {
      delete process.env.CRM_A_CONSOLE_WEB_BASE_URL;
    }
  });

  it("does NOT arm when no Crm-A Cloud API key is present", () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-trigger-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    ...
    expect(logs.some((m) => m.includes("No Crm-A Cloud API key"))).toBe(true);
  });
  ...
  it("does NOT fire an immediate tick on arm (waits for first interval)", async () => {
    ...
    // `crm-a-console update` when the gateway booted before the web runtime
  ...
  it("respects CRM_A_CONSOLE_WEB_BASE_URL env override", async () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-trigger-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    writeAuthProfiles(stateDir, "dc-key");
    process.env.CRM_A_CONSOLE_WEB_BASE_URL = "http://127.0.0.1:9999";
...
/**
 * Sync refresh tools (`crm_a_console_refresh_sync`, `crm_a_console_resync_full`)
 * — the user-facing escape hatch when someone says "refresh my inbox"
 * mid-conversation. Pinned here because:
 *
 *   1. They MUST register only when a Crm-A Cloud key is present —
 ...
 */
describe("crm-a-ai-gateway sync refresh tools", () => {
  const originalFetch = globalThis.fetch;
  const originalStateDir = process.env.OPENCLAW_STATE_DIR;
  let stateDir: string | undefined;
  ...
  it("registers both refresh tools when a Crm-A Cloud key is present", () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-refresh-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    writeAuthProfiles(stateDir, "dc-key");
    ...
    expect(names).toEqual(
      expect.arrayContaining(["crm_a_console_refresh_sync", "crm_a_console_resync_full"]),
    );
    expect(info).toHaveBeenCalledWith(expect.stringContaining("registered sync refresh tools"));
  });

  it("does NOT register refresh tools when no Crm-A Cloud key is configured", () => {
    ...
    const names = tools.map((tool: { name: string }) => tool.name);
    expect(names).not.toContain("crm_a_console_refresh_sync");
    expect(names).not.toContain("crm_a_console_resync_full");
    expect(info).not.toHaveBeenCalledWith(expect.stringContaining("sync refresh tools"));
  });

  it("crm_a_console_refresh_sync executes against /api/sync/refresh with mode incremental", async () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-refresh-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    writeAuthProfiles(stateDir, "dc-key");
    ...
    const refreshTool = tools.find(
      (tool: { name: string }) => tool.name === "crm_a_console_refresh_sync",
    );
  ...
  it("crm_a_console_resync_full executes against /api/sync/refresh with mode backfill", async () => {
    stateDir = mkdtempSync(path.join(os.tmpdir(), "crm-a-ai-gateway-refresh-"));
    process.env.OPENCLAW_STATE_DIR = stateDir;
    writeAuthProfiles(stateDir, "dc-key");
    writeWebRuntimeProcessFile(stateDir, 31000);
    ...
    const resyncTool = tools.find(
      (tool: { name: string }) => tool.name === "crm_a_console_resync_full",
    );
