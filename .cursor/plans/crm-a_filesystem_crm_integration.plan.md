---
name: Crm-A Filesystem CRM Integration
overview: Replace Crm-A's slow tool-by-tool CRM agent with a filesystem-first architecture where OpenClaw manages a DuckDB database and markdown documents in a dedicated workspace folder, synced via S3, and surfaced in Crm-A's sidebar.
todos:
  - id: inject-skill-infra
    ...
    status: pending
  - id: crm-a-skill
    content: "Phase 2: Create skills/crm-a/SKILL.md with DuckDB schema, SQL patterns, CRM patterns, document management instructions"
    status: pending
  - id: s3-sync-script
    content: "Phase 3: Create S3 sync script and sandbox startup hook for crm-a workspace persistence"
    status: pending
  - id: crm-a-sidebar
    content: "Phase 4 (DEFERRED): Add workspace data source to Crm-A sidebar (tRPC endpoints + buildKnowledgeTree merge)"
    status: pending
  - id: lambda-sync
...
# Crm-A Filesystem-First CRM via OpenClaw

## Architecture Overview

The current Crm-A CRM agent uses 15+ individual tRPC-backed tools (createObjectTool, createFieldTool, createEntryTool, searchEntriesTool, etc.) that each make a Prisma call. This is slow -- the agent often needs 10+ tool calls for a single user request.

The new architecture gives OpenClaw a `crm-a/` workspace folder with a DuckDB database and markdown files. The agent generates SQL directly via `exec` (duckdb CLI) and writes documents as `.md` files. S3 syncs this data between sandbox sessions, and Crm-A's sidebar reads from S3/PostgreSQL.
...
- PostgreSQL-compatible SQL dialect -- matches Crm-A's Supabase/Postgres, so generated SQL is portable
...
flowchart TB
  subgraph sandbox [OpenClaw Sandbox]
    ...
    skill[Crm-A Skill - always in context]
    ctx[crm-a/workspace_context.yaml]
    db[crm-a/workspace.duckdb]
    docs[crm-a/documents/*.md]
    agent --> skill
  ...
  subgraph crmAApp [Crm-A Web App]
    sidebar[App Sidebar]
    api[tRPC API]
...
Currently, all skills are lazy-loaded: only name + description appear in the system prompt, and the agent must `read` the SKILL.md to get full instructions. For the Crm-A CRM skill, we need the full content injected automatically.
...
## Phase 2: Crm-A CRM Skill (OpenClaw)

Create `skills/crm-a/SKILL.md` with:

- Full DuckDB schema reference
- nanoid 32 macro for ID generation (matching Crm-A's Supabase nanoid IDs)
...
- CRM patterns (contact, lead, deal, etc.) ported from the [Crm-A CRM agent prompt](file:///Users/kumareth/Documents/projects/crm-a/src/lib/agents/crm-agent.ts)

**New file:** `skills/crm-a/SKILL.md`
...
---
name: crm-a
description: Manage Crm-A CRM workspace - create objects, fields, entries via DuckDB and documents as markdown files
metadata:
  openclaw:
    inject: true
    emoji: "📊"
...
**workspace_context.yaml** -- read-only context the agent consumes on startup (written by Crm-A, never by the agent):

```yaml
# Crm-A Workspace Context (READ-ONLY)
# This file is generated by Crm-A and synced via S3.
# The agent reads this for organizational context but MUST NOT modify it.
# Changes flow from Crm-A UI -> S3 -> this file (on sandbox init).

workspace:
  version: 1

# Organization identity (synced from Crm-A on sandbox init)
organization:
  id: "org_abc123"
  name: "Acme Corp"
  slug: "acme-corp"
...
# Mirrors Crm-A's base-objects.ts immutable list
protected_objects:
  - name: "people"
...
# Populated by Crm-A when sandbox initializes from S3
integrations:
  connections: []
...
- **PostgreSQL dialect**: Generated SQL is directly portable to Crm-A's Supabase/Postgres

**Skill content structure** -- the full SKILL.md incorporates and adapts every section from Crm-A's existing CRM agent prompt ([src/lib/agents/crm-agent.ts](file:///Users/kumareth/Documents/projects/crm-a/src/lib/agents/crm-agent.ts)), rewritten for DuckDB/filesystem execution instead of tool calls:

### Section 1: Role and Workspace Startup

- Role: Crm-A CRM Management Agent operating via DuckDB and filesystem
- On every conversation: read `crm-a/workspace_context.yaml` (READ-ONLY) for org context, members, integrations, protected objects
- Initialize DuckDB if not exists: `duckdb crm-a/workspace.duckdb < schema.sql`
- Database path: `crm-a/workspace.duckdb`, documents path: `crm-a/documents/`
...
- **getOrganizationMembersTool -> READ workspace_context.yaml**: Members list is in `workspace_context.yaml` under `members:`. Read with `cat crm-a/workspace_context.yaml` and extract the members section. User fields store member IDs like `usr_abc123`.
...
- **richtext**: Rich text for Notes fields. Content stored as entry_field value (plain text / markdown). Displayed in Notion-style editor in Crm-A UI.
...
- Create document: `write` tool to create `crm-a/documents/<path>.md` + `INSERT INTO documents` with metadata
...
- Document tree mirrors filesystem: `crm-a/documents/projects/alpha.md` -> `file_path = 'projects/alpha.md'`
...
- NEVER modify `workspace_context.yaml` -- it is read-only context from Crm-A
...
- `mkdir -p crm-a/knowledge/{object_name}/` -- create directory
...
- `rm -rf crm-a/knowledge/{object_name}/`
...
2. **S3 -> Sandbox:** On sandbox startup, download the latest crm-a/ folder from S3

**Option A (simpler): Script-based sync**

- Add a `crm-a/sync.sh` script that the agent calls after mutations
- Uses AWS CLI: `aws s3 sync crm-a/ s3://crm-a-workspaces/{orgId}/`
...
- Background process watches crm-a/ for changes
...
**New file:** `skills/crm-a/sync.sh` (bundled with the skill, referenced by SKILL.md)

**Sandbox startup hook:**

- Add to OpenClaw's sandbox initialization: download crm-a workspace from S3 if it exists
- Could be a `BOOTSTRAP.md` instruction or a sandbox pre-warm step

---

## Phase 4: Crm-A Sidebar Integration

The Crm-A sidebar currently reads from Prisma (`getAll` objects + `getAll` documents). It needs a new data source for workspace-managed data.
...
**Files to modify in Crm-A:**

- [src/lib/trpc/routers/objects.ts](file:///Users/kumareth/Documents/projects/crm-a/src/lib/trpc/routers/objects.ts) -- Add `getWorkspaceObjects` endpoint that queries the synced SQLite data (either by reading crmA.db from S3, or from a PostgreSQL materialized view)
- [src/lib/trpc/routers/documents.ts](file:///Users/kumareth/Documents/projects/crm-a/src/lib/trpc/routers/documents.ts) -- Add `getWorkspaceDocuments` endpoint
- [src/components/app-sidebar.tsx](file:///Users/kumareth/Documents/projects/crm-a/src/components/app-sidebar.tsx) -- Merge workspace data into the `buildKnowledgeTree` function
...
**Alternative (simpler for v1):** Crm-A backend downloads workspace.duckdb from S3 on demand and queries it directly using `duckdb` Node.js bindings (`@duckdb/node-api`). No Lambda needed initially.
...
- Crm-A sidebar auto-refreshes when workspace data changes
- Bidirectional: Crm-A UI edits write back to S3, agent picks up changes on next sandbox start
...
- **Three-layer storage** (following Fintool pattern):
  - **YAML** (`workspace_context.yaml`): Read-only workspace identity, team members, integrations, defaults. Written by Crm-A, consumed by agent. Never modified by the agent -- data flows one-way from Crm-A UI -> S3 -> this file.
...
- **DuckDB over SQLite**: Native PIVOT/UNPIVOT is essential for the EAV data model (rendering custom-field entries as tables). PostgreSQL-compatible SQL dialect means generated SQL is portable to Crm-A's Supabase/Postgres. Native JSON type eliminates string parsing for enum_values/colors. Built-in CSV/Parquet import with auto-detection enables bulk CRM operations. FTS extension provides full-text search. ~50-100ms startup overhead is acceptable.
- **nanoid 32 IDs**: Matches Crm-A's Supabase PostgreSQL nanoid format. Generated via a DuckDB macro using `generate_series` + `random()` over the standard nanoid alphabet (`0-9A-Za-z_-`). 32 chars provides 192 bits of entropy.
...
- **Skill inject vs bootstrap file**: Using skill metadata `inject: true` is cleaner than adding a new bootstrap file type. It keeps the Crm-A instructions modular and version-controlled in the skills directory.
