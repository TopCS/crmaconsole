---
name: gateway-ws crm-a-console lock
overview: Migrate `apps/web` chat transport from CLI `--stream-json` processes to Gateway WebSocket while preserving the existing SSE API contract, then lock web to a single `crm-a-console` profile and disable workspace/profile switching (403 for disabled APIs). Add targeted web and bootstrap tests for the new behavior.
todos:
  ...
  - id: profile-default-lock
    content: Default web runtime profile resolution to crm-a-console in workspace.ts and ensure state/web-chat/workspace paths resolve under ~/.openclaw-crm-a-console.
    status: completed
  - id: api-lockdown
    content: Return 403 for profile/workspace mutation APIs and keep /api/profiles compatible with a single crm-a-console profile payload.
    status: completed
  - id: ui-single-profile
    ...
    status: completed
  - id: crm-a-path-update
    content: Update skills/crm-a/SKILL.md workspace path references to ~/.openclaw-crm-a-console/workspace.
    status: completed
  - id: web-tests
    content: Update/add apps/web tests covering WS transport behavior, API lock responses, and crm-a-console path resolution.
    status: completed
  - id: bootstrap-tests
...
# Migrate Web Chat to Gateway WS + Lock Crm-A Console Profile
...
- Force single-profile behavior in web runtime (`crm-a-console`), so workspace/chat/session paths resolve to `~/.openclaw-crm-a-console/*`.
...
- Update profile resolution in `[apps/web/lib/workspace.ts](apps/web/lib/workspace.ts)` so web runtime defaults to `crm-a-console` (without changing test-mode assumptions), ensuring state dir resolves to `~/.openclaw-crm-a-console` unless explicitly overridden.
...
- Update watcher ignore path in `[apps/web/next.config.ts](apps/web/next.config.ts)` to include crm-a-console state dir.
...
- Make `[apps/web/app/api/profiles/route.ts](apps/web/app/api/profiles/route.ts)` return a single effective `crm-a-console` profile payload for UI compatibility.
...
## Crm-A skill path update

- Replace `~/.openclaw/workspace` references with `~/.openclaw-crm-a-console/workspace` in `[skills/crm-a/SKILL.md](skills/crm-a/SKILL.md)`.
...
- Path behavior tests:
  - add/adjust targeted assertions in workspace resolver tests for crm-a-console state/web-chat/workspace directories.
- Bootstrap tests (new):
...
- Smoke-check workspace tree and web sessions resolve under `~/.openclaw-crm-a-console` with switching/creation controls disabled.
