...
- **Agent outbound via chat (NUOVO, questa sessione — commit `cad6a38f4`..`5ac5e4683`)**: estensione `crm-a-nlpearl-outbound` (tool `crm_a_phone_campaign`: upsert → Pearl NLPearl paused → audience → send/activate dietro `confirm: true`); route `/api/campaigns/phone` estesa (azione `upsert` + threading brief/criteria); schema `Voice Brief` su campaign + fix `CAMPAIGN_SEND_NEW_FIELDS` (`External ID`); propagazione `CRM_A_PHONE_WEBHOOK_SECRET` al gateway (docker-compose `env_file: .env`, `.env.example` documentato); skill `skills/nlpearl` (API v2 + tool); asset demo `DEMO-PRESENTATION-SCRIPT.md` + README updates.

### In sospeso (prossima sessione)
0. **Rendere visibile nell'istanza** — `docker compose up -d --build --force-recreate` per includere nell'immagine la card NLPearl (Integrations), la skill `nlpearl` (Skills) e l'estensione/tool; segreto in `.env` (caricato via `env_file`).
