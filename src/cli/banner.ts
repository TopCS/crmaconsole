...
// ---------------------------------------------------------------------------
// CRM-A CONSOLE ASCII art (figlet "ANSI Shadow" font, baked at build time)
// ---------------------------------------------------------------------------
const CRM_A_CONSOLE_ASCII = [
  " ██████╗██████╗ ███╗   ███╗       █████╗      ██████╗ ██████╗ ███╗   ██╗███████╗ ██████╗ ██╗     ███████╗",
  "██╔════╝██╔══██╗████╗ ████║      ██╔══██╗    ██╔════╝██╔═══██╗████╗  ██║██╔════╝██╔═══██╗██║     ██╔════╝",
  "██║     ██████╔╝██╔████╔██║█████╗███████║    ██║     ██║   ██║██╔██╗ ██║███████╗██║   ██║██║     █████╗",
  "██║     ██╔══██╗██║╚██╔╝██║╚════╝██╔══██║    ██║     ██║   ██║██║╚██╗██║╚════██║██║   ██║██║     ██╔══╝",
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  if (!rich) {
    return CRM_A_CONSOLE_ASCII.join("\n");
  }
  return renderGradientFrame(CRM_A_CONSOLE_ASCII, 0);
}
...
export async function emitCliBanner(version: string, options: BannerOptions = {}) {
  ...
  if (rich) {
    ...
    process.stdout.write(CRM_A_CONSOLE_ASCII.join("\n") + "\n");
  }
