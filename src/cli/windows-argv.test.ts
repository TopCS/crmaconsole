...
describe("normalizeWindowsArgv", () => {
  it("returns argv unchanged on non-windows platforms", () => {
    const argv = ["node", "crm-a-console", "status"];
    expect(
  ...
  it("removes duplicated node executable arguments on windows", () => {
    ...
    const argv = ["node", execPath, "C:\\repo\\crm-a-console.mjs", execPath, "status"];
    ...
    ).toEqual(["node", "C:\\repo\\crm-a-console.mjs", "status"]);
  });

  it("strips control chars and wrapping quotes before exec-path matching", () => {
    const execPath = "C:\\Program Files\\nodejs\\node.exe";
    const argv = ["node", `"\u0000${execPath}\u0000"`, "C:\\repo\\crm-a-console.mjs", "status"];
