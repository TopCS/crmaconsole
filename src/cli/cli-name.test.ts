...
describe("cli-name", () => {
  it("resolves known CLI names from argv[1]", () => {
    ...
    expect(resolveCliName(["node", "crm-a-console"])).toBe("crm-a-console");
  ...
  it("replaces CLI name in command prefixes while preserving package runner prefix", () => {
    expect(replaceCliName("openclaw status", "crm-a-console")).toBe("crm-a-console status");
    expect(replaceCliName("pnpm openclaw status", "crm-a-console")).toBe(
      "pnpm crm-a-console status",
    );
    expect(replaceCliName("npx crm-a-console status", "openclaw")).toBe("npx openclaw status");
  });

  it("keeps command unchanged when it does not start with a known CLI prefix", () => {
    expect(replaceCliName("echo openclaw status", "crm-a-console")).toBe("echo openclaw status");
