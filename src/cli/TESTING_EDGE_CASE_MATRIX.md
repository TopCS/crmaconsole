...
| `profile.ts`            | root vs command-local bootstrap profile flag                             | `crm-a-console --profile X bootstrap` and `crm-a-console bootstrap --profile X` resolve to identical profile env | `src/cli/profile.test.ts` (existing, expand)                      |
