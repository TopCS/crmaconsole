...
description: Connected app integration recipes for Crm-A Integrations (Gmail, Slack, GitHub, Notion, Google Calendar, Linear, Stripe, YouTube, and 500+ more)
---

# Crm-A Integrations

Use the **Crm-A Integrations tools** for all connected third-party app tasks in Crm-A Console.

## Two tools only

1. **`crm_a_search_integrations`** — Search for available integration tools by query and/or toolkit slug. Returns tool slugs, descriptions, full `input_schema`, connection status, and connected accounts.
2. **`crm_a_execute_integrations`** — Execute a tool by its `tool_slug` with `arguments` matching the `input_schema`. The gateway handles authentication and account selection automatically when only one account is connected.

## Workflow

1. Call `crm_a_search_integrations` with a query with EXACT search strings of what might help you find relevant tools. like "posthog project", "stripe subscription", etc. (optionally narrow with toolkit).
...
4. Call `crm_a_execute_integrations` with `tool_slug` and the correct `arguments`.
...
When `crm_a_search_integrations` shows `account_count > 1` for a toolkit:
...
3. Pass the chosen `connected_account_id` to `crm_a_execute_integrations`.
...
- Treat the live schema from `crm_a_search_integrations` as authoritative over any recipe table below.
...
When delegating, include: the `tool_slug`, the `arguments` object (copy shapes from the live `input_schema` returned by `crm_a_search_integrations`), and if applicable the `connected_account_id`.
